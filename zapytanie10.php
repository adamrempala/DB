...
<BODY>
<h2> Who from given location wrote most posts in given language? </h2>
...
$_SESSION['LOCALI'] = $_POST['LOCA'];
$_SESSION['LANGUA'] = $_POST['LING'];
...
if (!$conn) {
    ...
    $a = transliterateString($_POST['LOCA']);
    $a1 = transliterateString($_POST['LING']);
    $b = transliterateString($_POST['FRMM']);
    $c = transliterateString($_POST['TTOO']);
    $d = $_POST['REXP'];
    
    echo 'Results for data:<br>Location: '.$a.'<br>Language: '.$a1.'<br>From: '.$b.'<br>To: '.$c.'<br> Number of results: '.$d.'<br><br>';
    
    SetServerOutput($conn, true);
    
    sendOracle($conn, "BEGIN zapytanie_10('$a1', '$a', '$b', '$c', $d); END;", "Query failed");
